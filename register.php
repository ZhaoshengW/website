...
<?php
	...
	if(isset($_POST['register_btn'])){	
		...
		if(strcmp($password,$confirmedPassword)){
			...
			$sql = "INSERT INTO users(nickname,username,password,hobbies) VALUES('$nickname','$username','$password','$hobbies')";
...
<?php
		if(isset($_SESSION['messageUsername'])){
			echo $_SESSION['messageUsername'];
			unset($_SESSION['messageUsername']);
		}
		if(isset($_SESSION['messagePassword'])){
			echo '<br></br>'. $_SESSION['messagePassword'];
...
<p> Register <p> 
	<div>
		<form action="http://localhost:1234/register.php" method="post">
			...
			<input type = "text" name = "nickname" placeholder = "" id = "nicknameID">
			...
			<input type = "password" name = "confirmedPassword" placeholder = "" id = "confirmedPasswordID">
			...
			<select id="ageID" name="age">     
			  ...
			  <option value="youngAdult" selected>18-25</option>
			  <option value="adult">26-65</option>
			  <option value="elder">Above 65</option>
			  <option value="can'tTell">Can't tell</option>
			</select>
